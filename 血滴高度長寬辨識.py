...
def detect_red_regions_and_black(image_path):
    ...
    for contour, cx in sorted_contours:
        ...
        cx = int(M["m10"] / M["m00"])
        cy = int(M["m01"] / M["m00"])
        (text_w, text_h), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 1.7, 4)

        text_pos = (int(ex - text_w / 2), int(ey + text_h / 2))
        cv2.putText(image, text, text_pos, cv2.FONT_HERSHEY_SIMPLEX, 1.7, (255, 0, 0), 4)

        # ➤ Label below the ellipse
        label = f"Circle {circle_count}"
        (lw, lh), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
        label_pos = (cx - lw // 2, int(ey + major_axis / 2) + lh + 50)
        cv2.putText(image, label, label_pos, cv2.FONT_HERSHEY_SIMPLEX, 1.7, (0, 0, 255), 2)
...
# 使用範例（圖片路徑請換成你自己那張）
image_path = r"C:\Users\user\Downloads\65.png"
